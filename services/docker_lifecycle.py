...
class LifecycleMixin:
    ...
    async def _on_login_detected(self, name: str, current: Dict, prev: Dict) -> None:
        ...
        config_file_exists = await asyncio.to_thread(
            os.path.isfile, os.path.join(config_dir, f"onebot11_{uin}.json")
        )
        already_injected = await asyncio.to_thread(
            self._bs_inject_done, data_dir_base, name, uin
        )
        if already_injected:
            if config_file_exists:
                # NapCat 侧已注入：跳过步骤①和容器重启，但步骤②（BS 连接同步）仍需执行
                if bs_enabled:
                    injected_url = await asyncio.to_thread(
                        self._read_injected_bs_url, config_dir, uin
                    )
                    if injected_url:
                        try:
        ...
        if ws_url:
            try:
                ...
                asyncio.create_task(self._async_restart_container(name))
            except Exception as e:
                if bs_enabled:
                    self.release_port(bs_port)  # type: ignore[attr-defined]
    ...
    async def _async_sync_bs_connection(
        ...
        try:
            ...
            conn_config = {
                ...
                "description": existing_conn.get("description", f"Auto [{uin}]"),
            ...
            r = await botshepherd_manager.update_connection(name, final_config)
            if isinstance(r, dict) and not r.get("success", True):
                logger.warning("BS 连接同步结果: %s → %s", name, r.get("error", r))
            else:
                logger.info("BS 连接同步完成: %s → %s", name, r)
        except Exception as e:
            logger.error("BS 连接同步失败 (%s): %s", name, e)

    async def _async_restart_container(self, name: str) -> None:
        """注入后延迟重启容器，使 NapCat 加载新配置。"""
        await asyncio.sleep(5)
        try:
            from services.docker_async import async_docker_manager

            await async_docker_manager.restart_container(name)
            logger.info("注入后容器重启完成: %s（NapCat 将加载新 WS 配置）", name)
        except Exception as exc:
            logger.warning("注入后容器重启失败 %s: %s（手动重启可恢复）", name, exc)
